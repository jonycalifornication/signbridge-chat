...
const SENTENCE_EXAMPLES = [
    'Алма дәмді.',
    'Мен Айназ. Мен саған қалай көмектесемін',
...
class WidgetDemo {
    ...
    async loadPresets() {
        ...
        try {
            ...
            if (!response.ok) {
                this.renderPresetsError('Failed to load presets');
                return;
            }
            ...
            this.renderPresetsError('Error loading presets');
        }
    }
    ...
    renderSentenceExamples() {
        ...
        summary.innerHTML = `
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
                <span style="font-size: 12px; color: var(--text-tertiary); background: #E2E8F0; padding: 2px 8px; border-radius: 12px;">${SENTENCE_EXAMPLES.length}</span>
                <svg class="summary-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="transition: transform 0.2s ease; color: var(--text-tertiary); transform: rotate(180deg);">
                    <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
            </div>
        `;
        ...
        for (const sentence of SENTENCE_EXAMPLES) {
            ...
            btn.style.boxShadow = '0 1px 2px rgba(0,0,0,0.02)';
            btn.innerHTML = `
                <div style="font-weight: 500; font-size: 13px; color: var(--text-main); white-space: normal; overflow: visible; text-overflow: unset; line-height: 1.4;">
                    ${sentence}
                </div>
            `;
